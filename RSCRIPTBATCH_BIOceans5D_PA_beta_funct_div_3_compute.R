...
mclapply(files, function(f) {
        ...
        # ?fd_fric_intersect
        beta.div <- fundiversity::fd_fric_intersect(sp_com = comm_fdiv_PA[1:1000,c(4:length(comm_fdiv_PA))], traits = pcoa.scores[,1:4], stand = T)
        # str(beta.div)
        # format = cell 1, cell 2, FRic_intersect
        # calculate average per cell
        beta <- data.frame( beta.div %>% group_by(first_site) %>% summarise(beta.div = mean(FRic_intersect, na.rm = T)) )
        # Extract y and y from 'beta' first_site
        x <- as.numeric( do.call(rbind, strsplit(x = beta$first_site, split = "_", fixed = T))[,1] )
        y <- as.numeric( do.call(rbind, strsplit(x = beta$first_site, split = "_", fixed = T))[,2] )
        # Provide back to 'beta'
        beta$x <- x
        beta$y <- y
        # Rename to cell_id
        colnames(beta)[1] <- "cell_id"
        
        if( nrow(beta) == nrow(comm_fdiv_PA) ) {
            ...
            save(beta, file = filename)
                     
            ggplot() + geom_raster(aes(x = x, y = y, fill = ), data = beta) + scale_fill_viridis(name = "") +
                    geom_contour(colour = "grey30", binwidth = .1, size = 0.25, aes(x = x, y = y, z = Jtu), data = beta) +
        ...
        rm(dist.mat,indices,traits_fdiv,comm_fdiv,dat,beta.div,beta)
        gc()
