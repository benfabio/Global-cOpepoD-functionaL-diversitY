...
### Latest update: 10/06/24 (checking relatinships between copepod FD and MESOZOO normalized to NPP/CHL-a and NO2 concentrations - comments from Corentin Clerc)
...
### Adding new Suppl. Mat (Fig. SX) for the minor revisions of the paper:
### Plotting the PCoA space with species coordinates and correlation of the traits with PCoA scores as vectors.
...
pcoa <- wcmdscale(gow, eig = TRUE, add = TRUE) # Computes unweighted PCoA with cmdscale()
# str(pcoa)
# Get the PCoA axes scores
scores1 <- paste0("PCoA 1 (",floor(pcoa$eig[1]*100)/100,"%)")
scores2 <- paste0("PCoA 2 (",floor(pcoa$eig[2]*100)/100,"%)")
scores3 <- paste0("PCoA 3 (",floor(pcoa$eig[3]*100)/100,"%)")
scores4 <- paste0("PCoA 4 (",floor(pcoa$eig[4]*100)/100,"%)")

species_scores <- as.data.frame(pcoa$points)  # Extract species coordinates
colnames(species_scores)[c(1:5)] <- c("PCoA1","PCoA2","PCoA3","PCoA4","PCoA5") # Rename axes

# To understand how traits align with the PCoA axes: compute correlations between the traits and the PCoA scores.
# This will allow you to add trait vectors to the plot.
# Bur first, need to cretae dummy variables so categrical traits can be used for numerical correlation.
traits_dummy <- dummy_cols(traits_red2[,c(8,9,10,12:15,17:19)], select_columns = c("Spawning.mode"), remove_selected_columns = TRUE)
rownames(traits_dummy) <- rownames(traits_red2)
# dim(traits_dummy)
# head(traits_dummy)
# str(traits_dummy) # looks alright
# Correct dimensions?
# dim(traits_dummy); dim(species_scores) # looks good

# Remove NAs and compute corr coeff
traits_dummy <- na.omit(traits_dummy)
# dim(traits_dummy) # dim(species_scores[rownames(traits_dummy),c("PCoA1","PCoA2","PCoA3","PCoA4","PCoA5")])
corr.traits <- cor(traits_dummy, species_scores[rownames(traits_dummy),c("PCoA1","PCoA2","PCoA3","PCoA4","PCoA5")])
corr.traits

# Reshape for plotting
trait_vectors <- as.data.frame(corr.traits)
rownames(trait_vectors)[1] <- "Body length"
rownames(trait_vectors)[10] <- "Free\nspawning"
rownames(trait_vectors)[11] <- "Sac\nspawning"
trait_vectors$trait <- rownames(trait_vectors)  # Add trait names
# nice 

# PCoA plots with species scores
p1 <- ggplot(data = species_scores, aes(x = PCoA1, y = PCoA2)) +
    geom_hline(yintercept = 0, linetype = "dashed") +
    geom_vline(xintercept = 0, linetype = "dashed") +
    stat_density_2d(geom = "polygon", aes(alpha = (..level..)), fill = "grey") + 
    geom_point(colour = "#3288bd") +
    geom_segment(data = trait_vectors, aes(x = 0, y = 0, xend = PCoA1, yend = PCoA2), 
        arrow = arrow(length = unit(0.3,"cm")), color = "black", size = .4) +
    geom_text(data = trait_vectors, aes(x = PCoA1, y = PCoA2, label = trait),
        color = "black", hjust = 0.5, vjust = -0.5) +
    scale_x_continuous(limits = c(-0.85,0.85)) +
    scale_y_continuous(limits = c(-0.7,0.8)) +     
    xlab(scores1) + ylab(scores2) +
    theme_bw() + guides(alpha = "none")

p2 <- ggplot(data = species_scores, aes(x = PCoA3, y = PCoA4)) +
    geom_hline(yintercept = 0, linetype = "dashed") +
    geom_vline(xintercept = 0, linetype = "dashed") +
    stat_density_2d(geom = "polygon", aes(alpha = (..level..)), fill = "grey") + 
    geom_point(colour = "#3288bd") +
    geom_segment(data = trait_vectors, aes(x = 0, y = 0, xend = PCoA3, yend = PCoA4), 
        arrow = arrow(length = unit(0.3, "cm")), color = "black", size = .4) +
    geom_text(data = trait_vectors, aes(x = PCoA3, y = PCoA4, label = trait),
        color = "black", hjust = 0.5, vjust = -0.5) +
    scale_x_continuous(limits = c(-0.8,0.7)) +
    scale_y_continuous(limits = c(-0.65,0.7)) + 
    xlab(scores3) + ylab(scores4) +
    theme_bw() + guides(alpha = "none")

# Save as panel
require("ggpubr")
panel <- ggarrange(p1,p2, align = "hv", ncol = 2, nrow = 1, common.legend = T, labels = letters)
#panel
ggsave(plot = panel, filename = "Fig.X_trait_space_species+traits_19.11.24.jpg", dpi = 300, width = 10, height = 4.5)
...
# Compute monthly mean indices first
...
# Then annual mean and seasonality (std across month)
var.sr.faith <- data.frame(mon.sr.faith %>% group_by(cell_id) %>% 
        summarize(x = unique(x), y = unique(y),
            ann.faith = mean(mean.faith, na.rm = T), sd.faith = sd(mean.faith, na.rm = T), rel.sd.faith = (sd.faith*100)/ann.faith,
            ann.rich = mean(mean.rich, na.rm = T), sd.rich = sd(mean.rich, na.rm = T), rel.sd.rich = (sd.rich*100)/ann.rich)
)
...
var.ses <- data.frame(mon.ses %>% group_by(cell_id) %>% 
        summarize(x = unique(x), y = unique(y),
            ann.ses = mean(mean.ses, na.rm = T), sd.ses = sd(mean.ses, na.rm = T), rel.sd.ses = (sd.ses*100)/ann.ses)
)
...
var.beta <- data.frame(mon.beta %>% group_by(cell_id) %>% 
        summarize(x = unique(x), y = unique(y),
            ann.jac = mean(jac, na.rm = T), sd.jac = sd(jac, na.rm = T), rel.sd.jac = (sd.jac*100)/ann.jac,
            ann.jtu = mean(jtu, na.rm = T), sd.jtu = sd(jtu, na.rm = T), rel.sd.jtu = (sd.jtu*100)/ann.jtu,
            ann.jne = mean(jne, na.rm = T), sd.jne = sd(jne, na.rm = T), rel.sd.jne = (sd.jne*100)/ann.jne
        )
)
# summary(var.beta)
...
var.dbFD <- data.frame(mon.dbFD %>% group_by(cell_id) %>% 
        summarize(x = unique(x), y = unique(y),
            ann.feve = mean(FEve.avg, na.rm = T), sd.feve = sd(FEve.avg, na.rm = T), rel.sd.feve = (sd.feve*100)/ann.feve,
            ann.fdis = mean(FDis.avg, na.rm = T), sd.fdis = sd(FDis.avg, na.rm = T), rel.sd.fdis = (sd.fdis*100)/ann.fdis,
            ann.fdiv = mean(FDiv.avg, na.rm = T), sd.fdiv = sd(FDiv.avg, na.rm = T), rel.sd.fdiv = (sd.fdiv*100)/ann.fdiv
    )
)
# summary(var.dbFD)

### Maps to make: SR, Faith, SES Faith, FEve, FDis, FDiv, Jac, Jtu, Jne: 9x9 maps panel
map1 <- ggplot() + geom_raster(aes(x = x, y = y, fill = rel.sd.rich), data = var.sr.faith) +
    scale_fill_gradientn(name = "Intra-annual\nvariability\n(%Richness)", colours = parula(100), guide = "colourbar") +
    geom_contour(colour = "black", binwidth = 10, size = .4, aes(x = x, y = y, z = rel.sd.rich), data = var.sr.faith) +
    geom_polygon(aes(x = long, y = lat, group = group), data = world[world$long <= 180,], fill = "black", colour = "black", size = 0.3) +
    coord_quickmap() + theme(panel.background = element_rect(fill = "white"),legend.key = element_rect(fill = "grey50"),
    panel.grid.major = element_line(colour = "grey70",linetype = "dashed"), legend.position = "right") +
    scale_x_continuous(name = "", limits = c(-180,180), expand = c(0,0), labels = NULL) +
...
map2 <- ggplot() + geom_raster(aes(x = x, y = y, fill = rel.sd.faith), data = var.sr.faith) +
    scale_fill_gradientn(name = "Intra-annual\nvariability\n(%Faith)", colours = parula(100), guide = "colourbar") +
    geom_contour(colour = "black", binwidth = 5, size = .4, aes(x = x, y = y, z = rel.sd.faith), data = var.sr.faith) +
    geom_polygon(aes(x = long, y = lat, group = group), data = world[world$long <= 180,], fill = "black", colour = "black", size = 0.3) +
    coord_quickmap() + theme(panel.background = element_rect(fill = "white"),legend.key = element_rect(fill = "grey50"),
    panel.grid.major = element_line(colour = "grey70",linetype = "dashed"), legend.position = "right") +
    scale_x_continuous(name = "", limits = c(-180,180), expand = c(0,0), labels = NULL) +
...
map4 <- ggplot() + geom_raster(aes(x = x, y = y, fill = rel.sd.feve), data = var.dbFD) +
    scale_fill_gradientn(name = "Intra-annual\nvariability\n(%FEve)", colours = parula(100), guide = "colourbar") +
    geom_contour(colour = "black", binwidth = 10, size = .4, aes(x = x, y = y, z = rel.sd.feve), data = var.dbFD) +
    geom_polygon(aes(x = long, y = lat, group = group), data = world[world$long <= 180,], fill = "black", colour = "black", size = 0.3) +
    coord_quickmap() + theme(panel.background = element_rect(fill = "white"),legend.key = element_rect(fill = "grey50"),
    panel.grid.major = element_line(colour = "grey70",linetype = "dashed"), legend.position = "right") +
    scale_x_continuous(name = "", limits = c(-180,180), expand = c(0,0), labels = NULL) +
...
map5 <- ggplot() + geom_raster(aes(x = x, y = y, fill = rel.sd.fdis), data = var.dbFD) +
    scale_fill_gradientn(name = "Intra-annual\nvariability\n(%FDis)", colours = parula(100), guide = "colourbar") +
    geom_contour(colour = "black", binwidth = 1, size = .4, aes(x = x, y = y, z = rel.sd.fdis), data = var.dbFD) +
    geom_polygon(aes(x = long, y = lat, group = group), data = world[world$long <= 180,], fill = "black", colour = "black", size = 0.3) +
    coord_quickmap() + theme(panel.background = element_rect(fill = "white"),legend.key = element_rect(fill = "grey50"),
    panel.grid.major = element_line(colour = "grey70",linetype = "dashed"), legend.position = "right") +
    scale_x_continuous(name = "", limits = c(-180,180), expand = c(0,0), labels = NULL) +
...
map6 <- ggplot() + geom_raster(aes(x = x, y = y, fill = rel.sd.fdiv), data = var.dbFD) +
    scale_fill_gradientn(name = "Intra-annual\nvariability\n(%FDiv)", colours = parula(100), guide = "colourbar") +
    geom_contour(colour = "black", binwidth = 1, size = .4, aes(x = x, y = y, z = rel.sd.fdiv), data = var.dbFD) +
    geom_polygon(aes(x = long, y = lat, group = group), data = world[world$long <= 180,], fill = "black", colour = "black", size = 0.3) +
    coord_quickmap() + theme(panel.background = element_rect(fill = "white"),legend.key = element_rect(fill = "grey50"),
    panel.grid.major = element_line(colour = "grey70",linetype = "dashed"), legend.position = "right") +
    scale_x_continuous(name = "", limits = c(-180,180), expand = c(0,0), labels = NULL) +
...
map7 <- ggplot() + geom_raster(aes(x = x, y = y, fill = rel.sd.jac), data = var.beta) +
    scale_fill_gradientn(name = "Intra-annual\nvariability\n(%Jaccard)", colours = parula(100), guide = "colourbar") +
    geom_contour(colour = "black", binwidth = 5, size = .4, aes(x = x, y = y, z = rel.sd.jac), data = var.beta) +
    geom_polygon(aes(x = long, y = lat, group = group), data = world[world$long <= 180,], fill = "black", colour = "black", size = 0.3) +
    coord_quickmap() + theme(panel.background = element_rect(fill = "white"),legend.key = element_rect(fill = "grey50"),
    panel.grid.major = element_line(colour = "grey70",linetype = "dashed"), legend.position = "right") +
    scale_x_continuous(name = "", limits = c(-180,180), expand = c(0,0), labels = NULL) +
...
map8 <- ggplot() + geom_raster(aes(x = x, y = y, fill = rel.sd.jtu), data = var.beta) +
    scale_fill_gradientn(name = "Intra-annual\nvariability\n(%Turnover)", colours = parula(100), guide = "colourbar") +
    geom_contour(colour = "black", binwidth = 5, size = .4, aes(x = x, y = y, z = rel.sd.jtu), data = var.beta) +
    geom_polygon(aes(x = long, y = lat, group = group), data = world[world$long <= 180,], fill = "black", colour = "black", size = 0.3) +
    coord_quickmap() + theme(panel.background = element_rect(fill = "white"),legend.key = element_rect(fill = "grey50"),
    panel.grid.major = element_line(colour = "grey70",linetype = "dashed"), legend.position = "right") +
    scale_x_continuous(name = "", limits = c(-180,180), expand = c(0,0), labels = NULL) +
...
map9 <- ggplot() + geom_raster(aes(x = x, y = y, fill = rel.sd.jne), data = var.beta) +
    scale_fill_gradientn(name = "Intra-annual\nvariability\n(%Nestedness)", colours = parula(100), guide = "colourbar") +
    geom_contour(colour = "black", binwidth = 20, size = .4, aes(x = x, y = y, z = rel.sd.jne), data = var.beta) +
    geom_polygon(aes(x = long, y = lat, group = group), data = world[world$long <= 180,], fill = "black", colour = "black", size = 0.3) +
    coord_quickmap() + theme(panel.background = element_rect(fill = "white"),legend.key = element_rect(fill = "grey50"),
    panel.grid.major = element_line(colour = "grey70",linetype = "dashed"), legend.position = "right") +
    scale_x_continuous(name = "", limits = c(-180,180), expand = c(0,0), labels = NULL) +
...
ggsave(plot = figX, filename = "Fig.X_map_relative_sd_monthly_fd_indices_18.04.24.jpg", dpi = 300, width = 12, height = 12)
...
### 18/11/24: IMPLEMENTING MINOR REVISIONS - SHOW ALL REGIONS TOGETHER IN THE BOXPLOTS
### Variables to show:
# SR
# Faith
# FEve
...
b1 <- ggplot(data = fd2, aes(x = factor(region), y = SR, fill = factor(region))) +
    scale_fill_manual(name = "Regions", values = pal.clusters, guide = "colourbar") +
    geom_violin(colour = "black") + geom_boxplot(fill = "white", width = .2) + 
    xlab("Regions") + ylab("Species richness") + theme_bw()

b2 <- ggplot(data = fd2, aes(x = factor(region), y = Faith, fill = factor(region))) +
    scale_fill_manual(name = "Regions", values = pal.clusters, guide = "colourbar") +
    geom_violin(colour = "black") + geom_boxplot(fill = "white", width = .2) + 
    xlab("Regions") + ylab("Faith index") + theme_bw()

b3 <- ggplot(data = fd2, aes(x = factor(region), y = FEve, fill = factor(region))) +
    scale_fill_manual(name = "Regions", values = pal.clusters, guide = "colourbar") +
    geom_violin(colour = "black") + geom_boxplot(fill = "white", width = .2) + 
    xlab("Regions") + ylab("FEve") + theme_bw()

b4 <- ggplot(data = fd2, aes(x = factor(region), y = FDis, fill = factor(region))) +
    scale_fill_manual(name = "Regions", values = pal.clusters, guide = "colourbar") +
    geom_violin(colour = "black") + geom_boxplot(fill = "white", width = .2) + 
    xlab("Regions") + ylab("FDis") + theme_bw()

b5 <- ggplot(data = fd2, aes(x = factor(region), y = Jac, fill = factor(region))) +
    scale_fill_manual(name = "Regions", values = pal.clusters, guide = "colourbar") +
    geom_violin(colour = "black") + geom_boxplot(fill = "white", width = .2) + 
    xlab("Regions") + ylab("Trait dissimilarity\n(Jaccard)") + theme_bw()

b6 <- ggplot(data = fd2, aes(x = factor(region), y = log10(MESOZOO), fill = factor(region))) +
    scale_fill_manual(name = "Regions", values = pal.clusters, guide = "colourbar") +
    geom_violin(colour = "black") + geom_boxplot(fill = "white", width = .2) + 
    xlab("Regions") + ylab("Mesozooplankton biomass\nlog10(mgC.m-3)") + theme_bw()

b7 <- ggplot(data = fd2, aes(x = factor(region), y = log10(CHL), fill = factor(region))) +
    scale_fill_manual(name = "Regions", values = pal.clusters, guide = "colourbar") +
    geom_violin(colour = "black") + geom_boxplot(fill = "white", width = .2) + 
    xlab("Regions") + ylab("Chlorophyll a\nlog10(mgC.m-3)") + theme_bw()

b8 <- ggplot(data = fd2, aes(x = factor(region), y = log10(NPP), fill = factor(region))) +
    scale_fill_manual(name = "Regions", values = pal.clusters, guide = "colourbar") +
    geom_violin(colour = "black") + geom_boxplot(fill = "white", width = .2) + 
    xlab("Regions") + ylab("NPP\nlog10(mgC.m-2.d-1)") + theme_bw()

b9 <- ggplot(data = fd2, aes(x = factor(region), y = log10(FPOCex), fill = factor(region))) +
    scale_fill_manual(name = "Regions", values = pal.clusters, guide = "colourbar") +
    geom_violin(colour = "black") + geom_boxplot(fill = "white", width = .2) + 
    xlab("Regions") + ylab("FPOCex\nlog10(mgC.m-2.d-1)") + theme_bw()

b10 <- ggplot(data = fd2, aes(x = factor(region), y = log10(PROCHL), fill = factor(region))) +
    scale_fill_manual(name = "Regions", values = pal.clusters, guide = "colourbar") +
    geom_violin(colour = "black") + geom_boxplot(fill = "white", width = .2) + 
    xlab("Regions") + ylab("Prochlorococcus biomass\nlog10(mgC.m-3)") + theme_bw()

# Assemble in panel and save
panel <- ggarrange(b1,b2,b3,b4,b5,b6,b7,b8,b9,b10, ncol = 3, nrow = 4, align = "hv", labels = letters)

setwd("/net/kryo/work/fabioben/BIOCeans5D/plots/")
ggsave(plot = panel, filename = "Fig.S12_panel_boxplots_all_regions_revised_18.11.24.jpg", height = 12, width = 10.5, dpi = 300)
...
### Plotting PCoA space
library("vegan")
pcoa2 <- cmdscale(d = gow, eig = T, add = TRUE)
...
### 10/06/2024: Examining the relationships between FD facets and MESOZOO after standardizing per units of resource availability (CHL-a, NPP or N03 concentrations)
fd <- get(load("table_mean_ann_FD_indices_baseline+BCP+biom_22.11.23.RData"))
# str(fd); colnames(fd)
# Could normalize to units of: CHL, MICRO, NANO, PICO, NPP and NPPv2 (v2 better because more complete spatially)
# fd$MESOZOO_CHL <- fd$MESOZOO/fd$CHL
# fd$MESOZOO_MICRO <- fd$MESOZOO/fd$MICRO
# fd$MESOZOO_NANO <- fd$MESOZOO/fd$NANO
# fd$MESOZOO_PICO <- fd$MESOZOO/fd$PICO
# #fd$MESOZOO_NPP <- fd$MESOZOO/fd$NPP
# fd$MESOZOO_NPP <- fd$MESOZOO/fd$NPPv2
# # summary(fd)
# # Check their distribution
# p1 <- ggplot(fd, aes(x = log10(MESOZOO_CHL))) + geom_histogram(binwidth = .01, colour = "black", fill = "white")
# p2 <- ggplot(fd, aes(x = log10(MESOZOO_MICRO))) + geom_histogram(binwidth = .01, colour = "black", fill = "white")
# p3 <- ggplot(fd, aes(x = log10(MESOZOO_NANO))) + geom_histogram(binwidth = .01, colour = "black", fill = "white")
# p4 <- ggplot(fd, aes(x = log10(MESOZOO_PICO))) + geom_histogram(binwidth = .01, colour = "black", fill = "white")
# #p5 <- ggplot(fd, aes(x = log10(MESOZOO_NPP))) + geom_histogram(binwidth = .01, colour = "black", fill = "white")
# p5 <- ggplot(fd, aes(x = log10(MESOZOO_NPPv2))) + geom_histogram(binwidth = .01, colour = "black", fill = "white")
# require("ggpubr")
# ggarrange(p1,p2,p3,p4,p5, align = "hv")
#
# # Global maps of standardized products
# m2 <- ggplot() + geom_raster(aes(x = x, y = y, fill = MESOZOO_CHL), data = fd) +
#     scale_fill_gradientn(name = "MESOZOO/CHL-A", colours = parula(100), guide = "colourbar") +
#     geom_contour(colour = "black", binwidth = 5, size = .4, aes(x = x, y = y, z = MESOZOO_CHL), data = fd) +
#     geom_polygon(aes(x = long, y = lat, group = group), data = world[world$long <= 180,], fill = "black", colour = "black", size = 0.3) +
#     coord_quickmap() + theme(panel.background = element_rect(fill = "white"),legend.key = element_rect(fill = "grey50"),
#     panel.grid.major = element_line(colour = "grey70",linetype = "dashed"), legend.position = "right") +
#     scale_x_continuous(name = "", limits = c(-180,180), expand = c(0,0), labels = NULL) +
#     scale_y_continuous(name = "", limits = c(-90,90), expand = c(0,0), labels = NULL)
#
# m1 <- ggplot() + geom_raster(aes(x = x, y = y, fill = log10(MESOZOO_NPP)), data = fd) +
#     scale_fill_gradientn(name = "MESOZOO/NPP\n(log10)", colours = parula(100), guide = "colourbar") +
#     geom_contour(colour = "black", binwidth = .20, size = .4, aes(x = x, y = y, z = log10(MESOZOO_NPP)), data = fd) +
#     geom_polygon(aes(x = long, y = lat, group = group), data = world[world$long <= 180,], fill = "black", colour = "black", size = 0.3) +
#     coord_quickmap() + theme(panel.background = element_rect(fill = "white"),legend.key = element_rect(fill = "grey50"),
#     panel.grid.major = element_line(colour = "grey70",linetype = "dashed"), legend.position = "right") +
#     scale_x_continuous(name = "", limits = c(-180,180), expand = c(0,0), labels = NULL) +
#     scale_y_continuous(name = "", limits = c(-90,90), expand = c(0,0), labels = NULL)
#
# m3 <- ggplot() + geom_raster(aes(x = x, y = y, fill = MESOZOO_MICRO), data = fd) +
#     scale_fill_gradientn(name = "MESOZOO/DIATO", colours = parula(100), guide = "colourbar") +
#     geom_contour(colour = "black", binwidth = 20, size = .4, aes(x = x, y = y, z = MESOZOO_MICRO), data = fd) +
#     geom_polygon(aes(x = long, y = lat, group = group), data = world[world$long <= 180,], fill = "black", colour = "black", size = 0.3) +
#     coord_quickmap() + theme(panel.background = element_rect(fill = "white"),legend.key = element_rect(fill = "grey50"),
#     panel.grid.major = element_line(colour = "grey70",linetype = "dashed"), legend.position = "right") +
#     scale_x_continuous(name = "", limits = c(-180,180), expand = c(0,0), labels = NULL) +
#     scale_y_continuous(name = "", limits = c(-90,90), expand = c(0,0), labels = NULL)
#
# m4 <- ggplot() + geom_raster(aes(x = x, y = y, fill = MESOZOO_NANO), data = fd) +
#     scale_fill_gradientn(name = "MESOZOO/NANO", colours = parula(100), guide = "colourbar") +
#     geom_contour(colour = "black", binwidth = 20, size = .4, aes(x = x, y = y, z = MESOZOO_NANO), data = fd) +
#     geom_polygon(aes(x = long, y = lat, group = group), data = world[world$long <= 180,], fill = "black", colour = "black", size = 0.3) +
#     coord_quickmap() + theme(panel.background = element_rect(fill = "white"),legend.key = element_rect(fill = "grey50"),
#     panel.grid.major = element_line(colour = "grey70",linetype = "dashed"), legend.position = "right") +
#     scale_x_continuous(name = "", limits = c(-180,180), expand = c(0,0), labels = NULL) +
#     scale_y_continuous(name = "", limits = c(-90,90), expand = c(0,0), labels = NULL)
#
# m5 <- ggplot() + geom_raster(aes(x = x, y = y, fill = MESOZOO_PICO), data = fd) +
#     scale_fill_gradientn(name = "MESOZOO/PICO", colours = parula(100), guide = "colourbar") +
#     geom_contour(colour = "black", binwidth = 20, size = .4, aes(x = x, y = y, z = MESOZOO_PICO), data = fd) +
#     geom_polygon(aes(x = long, y = lat, group = group), data = world[world$long <= 180,], fill = "black", colour = "black", size = 0.3) +
#     coord_quickmap() + theme(panel.background = element_rect(fill = "white"),legend.key = element_rect(fill = "grey50"),
#     panel.grid.major = element_line(colour = "grey70",linetype = "dashed"), legend.position = "right") +
#     scale_x_continuous(name = "", limits = c(-180,180), expand = c(0,0), labels = NULL) +
#     scale_y_continuous(name = "", limits = c(-90,90), expand = c(0,0), labels = NULL)
#
# map.arrange <- ggarrange(m1,m2,m3,m4,m5, ncol = 2, nrow = 3, align = "hv")
# ggsave(plot = map.arrange, filename = "panel_maps_standardized_MESOZOO_estimates_10_10_24.pdf", dpi = 300, height = 7, width = 13)
#
#
# ### Perform PCA to examine covariance with FD facets (keep MESOZOO)
# names2keep <- colnames(fd)[c(4:9,11,12,14,16,40:45)]; names2keep
# data4pca <- fd[,c("x","y",names2keep)]
# # Log transform - TRY ALSO WITHOUT LOG TRANSFORMING
# #data4pca[,c(13:18)] <- log10(data4pca[,c(13:18)])
# colnames(data4pca)[3:12] <- c("Richness","Faith","SES Faith","Trait dissim.","Trait turnover","Trait nestedness","FEve","FDis","FDiv","Rao's Q")
# data4pca <- na.omit(data4pca)
# pca <- PCA(X = data4pca[,c(3:length(data4pca))], scale.unit = T, graph = F, ncp = 5, quanti.sup = c(11:16))
# # summary(pca)
# #                        Dim.1   Dim.2  Dim.3
# # Variance               6.858   1.640  0.779
# # % of var.             68.575  16.396  7.790
# # Cumulative % of var.  68.575  84.971  92.76
# # plot.PCA(pca, axes = c(1,2), choix = "var")
# # plot.PCA(pca, axes = c(2,3), choix = "var")
#
# ### MESOZOO standardized to CHL-a and NANO/MICRO INCREASES with SR, FR and FDis
# ### BUT: MESOZOO standardized to NPP or PICO and NANO/MICRO DECREASES with SR, FR and FDis...
# eig <- data.frame(perc = pca$eig[,"percentage of variance"], nb = c(1:nrow(pca$eig)))
# pca1 <- paste0("PC1 (",floor(eig$perc[1]*100)/100,"%)")
# pca2 <- paste0("PC2 (",floor(eig$perc[2]*100)/100,"%)")
# #pca3 <- paste0("PC3 (",floor(eig$perc[3]*100)/100,"%)")
# # Function for nicer pca plots
...
# pcad <- augment.PCA(pca)
#
# pca.better <- ggplot(pcad, aes(colour = factor(type))) + coord_fixed() +
#   annotate(geom="path", colour="black", x=cos(seq(0, 2*pi, length.out=100)), y=sin(seq(0, 2*pi, length.out=100)), colour = "grey30") +
#   geom_segment(aes(x=0, xend = Dim.1, y=0, yend = Dim.2), arrow=arrow(angle=20, length=unit(0.01,"npc"))) +
#   scale_colour_manual(name = "", values = c("#c51b7d","#276419"), guide = "none") +
#   geom_text_repel(aes(x=Dim.1, y=Dim.2, label=var)) + xlab(pca1) + ylab(pca2) + theme_bw()
#
# # Maps of PCs
# data4pca[,c("PC1","PC2")] <- pca$ind$coord[,c(1:2)]
#
...
#     scale_y_continuous(name = "", limits = c(-90,90), expand = c(0,0), labels = NULL)
#
# panel <- ggarrange(map1,map2,pca.better, align = "v", nrow = 2, ncol = 2)
# ggsave(plot = panel, filename = "panel_PCs_standardized_MESOZOO_estimates_10_10_24.pdf", dpi = 300, height = 8, width = 12)
    

### 17/06/24: After discussing with Corentin & Jonas & Nielja, stick to MESOZOO/NPP ratio vs. FD indices
fd$MESOZOO_NPP <- fd$MESOZOO/fd$NPPv2

# For adding linear fots to the plots below if you want to
library("ggpmisc")
formula1 <- y ~ x 
formula2 <- y ~ poly(x,2)

p1 <- ggplot(fd, aes(x = SR, y = log10(MESOZOO_NPP), colour = abs(y))) +
  geom_point(alpha = .3) + geom_smooth(colour = "black", method = "lm", formula = formula1) +
  scale_colour_gradientn(name = "Latitude", colours = rev(parula(100)), guide = "colourbar") +
  stat_poly_eq(use_label(c("eq","adj.R2","P")), formula = formula1, label.y = "top", label.x = "right", size = 3, colour = "black") + 
  xlab("Species richness") + ylab("MESOZOO:NPP (log10)") + theme_bw() 

p2 <- ggplot(fd, aes(x = Faith, y = log10(MESOZOO_NPP), colour = abs(y))) +
  geom_point(alpha = .3) + geom_smooth(colour = "black", method = "lm", formula = formula1) +
  scale_colour_gradientn(name = "Latitude", colours = rev(parula(100)), guide = "colourbar") +
  stat_poly_eq(use_label(c("eq","adj.R2","P")), formula = formula1, label.y = "top", label.x = "left", size = 3, colour = "black") + 
  xlab("Faith") + ylab("MESOZOO:NPP (log10)") + theme_bw() 

p3 <- ggplot(fd, aes(x = FEve, y = log10(MESOZOO_NPP), colour = abs(y))) +
  geom_point(alpha = .3) + geom_smooth(colour = "black", method = "lm", formula = formula1) +
  scale_colour_gradientn(name = "Latitude", colours = rev(parula(100)), guide = "colourbar") +
  stat_poly_eq(use_label(c("eq","adj.R2","P")), formula = formula1, label.y = "top", label.x = "right", size = 3, colour = "black") + 
  xlab("FEve") + ylab("MESOZOO:NPP (log10)") + theme_bw() 

p4 <- ggplot(fd, aes(x = FDis, y = log10(MESOZOO_NPP), colour = abs(y))) +
  geom_point(alpha = .3) + geom_smooth(colour = "black", method = "lm", formula = formula1) +
  scale_colour_gradientn(name = "Latitude", colours = rev(parula(100)), guide = "colourbar") +
  stat_poly_eq(use_label(c("eq","adj.R2","P")), formula = formula1, label.y = "top", label.x = "left", size = 3, colour = "black") + 
  xlab("FDis") + ylab("MESOZOO:NPP (log10)") + theme_bw() 

p5 <- ggplot(fd, aes(x = FDiv, y = log10(MESOZOO_NPP), colour = abs(y))) +
  geom_point(alpha = .3) + geom_smooth(colour = "black", method = "lm", formula = formula1) +
  scale_colour_gradientn(name = "Latitude", colours = rev(parula(100)), guide = "colourbar") +
  stat_poly_eq(use_label(c("eq","adj.R2","P")), formula = formula1, label.y = "top", label.x = "right", size = 3, colour = "black") + 
  xlab("FDiv") + ylab("MESOZOO:NPP (log10)") + theme_bw() 

p6 <- ggplot(fd, aes(x = Jac, y = log10(MESOZOO_NPP), colour = abs(y))) +
  geom_point(alpha = .3) + geom_smooth(colour = "black", method = "lm", formula = formula1) +
  scale_colour_gradientn(name = "Latitude", colours = rev(parula(100)), guide = "colourbar") +
  stat_poly_eq(use_label(c("eq","adj.R2","P")), formula = formula1, label.y = "top", label.x = "left", size = 3, colour = "black") + 
  xlab("Trait dissimilarity (Jaccard)") + ylab("MESOZOO:NPP (log10)") + theme_bw() 
    
panel <- ggarrange(p1,p2,p3,p4,p5,p6, align = "hv", ncol = 2, nrow = 3, labels = letters)
ggsave(plot = panel, filename = "Fig.X_panel_fd_MESOZOO:NPP_17.06.24.pdf", dpi = 300, height = 14, width = 12)
ggsave(plot = panel, filename = "Fig.X_panel_fd_MESOZOO:NPP_17.06.24.jpg", dpi = 300, height = 14, width = 12)
    

### Check by log transforming NOO before computing the ratio
# fd$MESOZOO_logNPP <- fd$MESOZOO/log10(fd$NPPv2)
#
# ggplot(fd, aes(x = SR, y = MESOZOO_logNPP, colour = abs(y))) +
#   geom_point() + geom_smooth(colour = "black", method = "lm", formula = formula1) +
#   scale_colour_gradientn(name = "Latitude", colours = rev(parula(100)), guide = "colourbar") +
#   stat_poly_eq(use_label(c("eq","adj.R2","P")), formula = formula1, label.y = "top", label.x = "right", size = 3, colour = "black") +
#   xlab("Species richness") + ylab("MESOZOO/logged NPP") + theme_bw()
#
# ggplot(fd, aes(x = Faith, y = MESOZOO_logNPP, colour = abs(y))) +
#   geom_point() + geom_smooth(colour = "black", method = "lm", formula = formula1) +
#   scale_colour_gradientn(name = "Latitude", colours = rev(parula(100)), guide = "colourbar") +
#   stat_poly_eq(use_label(c("eq","adj.R2","P")), formula = formula1, label.y = "top", label.x = "right", size = 3, colour = "black") +
#   xlab("Faith") + ylab("MESOZOO/logged NPP") + theme_bw()
#
# ggplot(fd, aes(x = FEve, y = MESOZOO_logNPP, colour = abs(y))) +
#   geom_point() + geom_smooth(colour = "black", method = "lm", formula = formula1) +
#   scale_colour_gradientn(name = "Latitude", colours = rev(parula(100)), guide = "colourbar") +
#   stat_poly_eq(use_label(c("eq","adj.R2","P")), formula = formula1, label.y = "top", label.x = "right", size = 3, colour = "black") +
#   xlab("FEve") + ylab("MESOZOO/logged NPP") + theme_bw()
#
# ggplot(fd, aes(x = FDis, y = MESOZOO_logNPP, colour = abs(y))) +
#   geom_point() + geom_smooth(colour = "black", method = "lm", formula = formula1) +
#   scale_colour_gradientn(name = "Latitude", colours = rev(parula(100)), guide = "colourbar") +
#   stat_poly_eq(use_label(c("eq","adj.R2","P")), formula = formula1, label.y = "top", label.x = "right", size = 3, colour = "black") +
#   xlab("FDis") + ylab("MESOZOO/logged NPP") + theme_bw()
#
# ggplot(fd, aes(x = Jac, y = MESOZOO_logNPP, colour = abs(y))) +
#   geom_point() + geom_smooth(colour = "black", method = "lm", formula = formula1) +
#   scale_colour_gradientn(name = "Latitude", colours = rev(parula(100)), guide = "colourbar") +
#   stat_poly_eq(use_label(c("eq","adj.R2","P")), formula = formula1, label.y = "top", label.x = "right", size = 3, colour = "black") +
#   xlab("Total trait dissimilarity\n(Jaccard)") + ylab("MESOZOO/logged NPP") + theme_bw()
